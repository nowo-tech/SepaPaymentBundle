...
namespace Nowo\SepaPaymentBundle\Tests\Unit\Service;
...
class MandateServiceTest extends TestCase
{
    ...
    public function testIsValidSequenceTransition(): void
    {
        ...
        $this->assertFalse($this->service->isValidSequenceTransition('UNKNOWN', 'RCUR'));
    }
    ...
    /**
     * Tests findMandatesByDebtorIban returns empty array when no mandates exist for the IBAN.
     */
    public function testFindMandatesByDebtorIbanReturnsEmptyWhenNoMandates(): void
    {
        $mandates = $this->service->findMandatesByDebtorIban('ES9121000418450200051332');
    ...
    public function testReactivateMandateThrowsWhenMandateExpired(): void
    {
        $this->service->createMandate(
            'MANDATE-EXPIRED',
            new DateTime('2020-01-01'),
        ...
        $this->service->suspendMandate('MANDATE-EXPIRED');

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Cannot reactivate an expired mandate');

        $this->service->reactivateMandate('MANDATE-EXPIRED');
    }
}
