...
class SepaCreditorIdentifierValidatorTest extends TestCase
{
    ...
    {
        $ref    = new ReflectionClass(SepaCreditorIdentifierValidator::class);
        $method = $ref->getMethod('validateCheckDigits');
        $this->assertFalse($method->invoke($this->validator, 'ES', '00', 'M12345678'));
        $this->assertTrue($method->invoke($this->validator, 'ES', '96', 'M12345678'));
    }
    ...
    /**
     * Covers isValid when country code format is invalid (not 2 letters).
     */
    public function testIsValidWithInvalidCountryCodeFormat(): void
    {
        $this->assertFalse($this->validator->isValid('E97ZZZM12345678'));
        $this->assertFalse($this->validator->isValid('9S97ZZZM12345678'));
    }

    /**
     * Covers isValid when check digits format is invalid (not 2 digits).
     */
    public function testIsValidWithInvalidCheckDigitsFormat(): void
    {
        $this->assertFalse($this->validator->isValid('ES9ZZZZM12345678'));
        $this->assertFalse($this->validator->isValid('ES9XZZZM12345678'));
    }

    /**
     * Covers isValid when length exceeds MAX_LENGTH (35 chars).
     */
    public function testIsValidWithLengthExceedsMax(): void
    {
        $long = 'ES97ZZZ' . str_repeat('A', 29);
        $this->assertSame(36, strlen($long));
        $this->assertFalse($this->validator->isValid($long));
    }

    /**
     * Covers isValid when national identifier contains invalid character (not alphanumeric).
     */
    public function testIsValidWithInvalidNationalIdentifierFormat(): void
    {
        $this->assertFalse($this->validator->isValid('ES97ZZZM1234567-'));
    }

    /**
     * Covers isValid when suffix format is invalid (not 3 alphanumeric chars).
     */
    public function testIsValidWithInvalidSuffixFormat(): void
    {
        $this->assertFalse($this->validator->isValid('ES97ZZ-M12345678'));
    }
}
