...
namespace Nowo\SepaPaymentBundle\Tests\Unit\Lookup;
...
class BicLookupServiceTest extends TestCase
{
    ...
    /**
     * Tests lookupBicByCountry (private) DE branch via reflection (8-digit bank code).
     */
    public function testLookupBicByCountryDeBranchViaReflection(): void
    {
        $ref    = new ReflectionClass(BicLookupService::class);
        $method = $ref->getMethod('lookupBicByCountry');

        $bic = $method->invoke($this->lookupService, 'DE', '700202701234567890');
        $this->assertSame('HYVEDEMM', $bic);
    }

    /**
     * Tests lookupBicByCountry (private) BE branch via reflection (3-digit bank code).
     */
    public function testLookupBicByCountryBeBranchViaReflection(): void
    {
        $ref    = new ReflectionClass(BicLookupService::class);
        $method = $ref->getMethod('lookupBicByCountry');

        $bic = $method->invoke($this->lookupService, 'BE', '00123456789012');
        $this->assertSame('GEBABEBB', $bic);
    }

    /**
     * Tests lookupBicByCountry (private) FR branch via reflection (5-digit bank code).
     */
    public function testLookupBicByCountryFrBranchViaReflection(): void
    {
        $ref    = new ReflectionClass(BicLookupService::class);
        $method = $ref->getMethod('lookupBicByCountry');
    ...
    /**
     * Tests lookupBicByCountry (private) IT branch via reflection (5-digit bank code).
     */
    public function testLookupBicByCountryItBranchViaReflection(): void
    {
        $ref    = new ReflectionClass(BicLookupService::class);
        $method = $ref->getMethod('lookupBicByCountry');
    ...
    /**
     * Tests lookupBicByCountry (private) default branch via reflection (country not in switch).
     */
    public function testLookupBicByCountryDefaultBranchViaReflection(): void
    {
        $ref    = new ReflectionClass(BicLookupService::class);
        $method = $ref->getMethod('lookupBicByCountry');
