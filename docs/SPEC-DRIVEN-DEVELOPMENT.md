...
2. **Product behavior** — what **SepaPaymentBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** finance integrator, **I want** pain.001/pain.008 XML generation **so that** SEPA credit transfers and direct debits export to the bank. |
| US-02 | **As an** integrator, **I want** IBAN/BIC validation and mandate storage **so that** invalid payments fail before export. |
| US-03 | **As an** integrator, **I want** ISO 20022-compliant field mapping **so that** files pass bank validation. |
| US-04 | **As a** maintainer, **I want** PHPUnit covering XML builders **so that** schema regressions are caught in CI. |
...
**Goal:** Symfony bundle for SEPA payment management: ISO 20022 XML (pain.001 credit transfer, pain.008 direct debit), validation, mandates, export, and banking utilities.
